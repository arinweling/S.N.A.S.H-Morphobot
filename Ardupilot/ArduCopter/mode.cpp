...
bool Copter::set_mode(Mode::Number mode, ModeReason reason)
{
    ...
    if(flightmode == &mode_drive)
    copter.was_drive_mode = true;
    else
    copter.was_drive_mode = false;

    if(flightmode == &mode_morph)
    {
...
Mode::Number Mode::get_previous_mode(Mode::Number curr_mode) { return curr_mode; }
